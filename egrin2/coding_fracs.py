...
def main():
	...
	op.add_option('-c', '--cache_dir', help='The cmpython cache directory where the genome features and info live', default='cache')
	op.add_option('-f', '--features_file', help='The sequence file (with coding regions) (found in cache/<organism name>features')
	op.add_option('-m', '--num_runs', help='The number of cmonkey runs', default=100)
	op.add_option('-n', '--num_cores', help='Number of cores to use on cluster', default=2)
